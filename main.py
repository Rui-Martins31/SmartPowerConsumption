...
# METHODS
#-----------------------------------------------------
def predict_power_consumption(day: datetime, hour: int) -> float:
    """
    Placeholder for the ML model. 
    Returns a dummy consumption value in kWh.
    """

    df = pd.read_csv('./MODEL_POWER_CONSUMPTION/DATASET/dataset_power_visualizer.csv', 
                     index_col=0, 
                     parse_dates=['Date'])
    
    lookup_datetime = day.replace(year=2007, hour=hour)

    return float(df.loc[lookup_datetime, 'Global_active_power'])



def start_day(curr_day: datetime):
    """
    ...
    for hour in range(24):
        pow_con: float = predict_power_consumption(day=curr_day, hour=hour)
...
def main() -> None:
    ...
    for day in range(num_days_sim):
        ...
        price_list, consump_list = start_day(curr_day = curr_date)
